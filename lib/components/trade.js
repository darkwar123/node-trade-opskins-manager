...
TradeOpskinsManager.prototype.acceptOffer = function({id}) {
	return new Promise((resolve, reject) => {
		if (typeof id === 'undefined') {
			throw new OpskinsError({"message": 'offer.id is required'});
		}

		let query = {"httpMethod": 'POST', "iface": 'ITrade', "method": 'AcceptOffer', "data": {"offer_id": id, "twofactor_code": this.getTwoFactorCode()}};

		this._req(query).then(({data}) => {
				if (data['status'] !== 1) {
					return reject(new OpskinsError({"message": data['message'], "statusCode": data['status']}));
				}

				resolve(new Offer(data['response']['offer']));
			})
			.catch(reject);
	});
};
